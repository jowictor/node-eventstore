...
_.extend(Mongo.prototype, {
  connect: function (callback) {
    ...
    if (options.url) {
      ...
      var memberString = options.dbHost;
    ...
    if (mongo.MongoClient.length === 3) {
      ensureIndex = "createIndex";
      client = new mongo.MongoClient.connect(connectionUrl);
      ...
      client.connect(options.url).then(function (instance, err) {
        if (err) {
          debug(err);
          if (callback) callback(err);
          return;
        ...
        self.db = instance.db(options.dbName);
        initDb();
      });
    }
